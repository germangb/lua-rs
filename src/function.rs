...
// implement up to 10 return values
    ...
    F: LuaFunction,
{
    fn into_lua(self, state: &mut LuaState) {
        unsafe {
            ...
            extern fn function<F>(state: *mut ffi::lua_State) -> ::std::os::raw::c_int
            where
                F: LuaFunction,
            {
                let mut state = LuaState { owned: false, lua_state: state };
